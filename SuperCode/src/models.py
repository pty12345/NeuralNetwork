...
# LPC-AD KDD2022
class LPC_AD(nn.Module):
	def __init__(self, feats, w1: int = 10, w2: int = 2,  latent_z: int = 8, rand: bool = True, f_theta='seq2seq-att'):
		super(LPC_AD, self).__init__()
		self.name = 'LPC_AD'
		self.device = 'cuda:0'
		self.lr = 0.001
		self.batch = 64
		...
		self.n_feats = feats
		self.seed = 120
		self.latent_z = latent_z # WADI is 16
		self.window1 = w1
		self.window2 = w2
		self.n_window = self.window1 + self.window2
		self.rand = rand                                # random perturb
		self.f_theta = f_theta

  
        # Model's submodels
		self.encoder = nn.LSTM(input_size=feats, hidden_size=feats // 2, num_layers=1, batch_first=True, bias=True)
		self.dense1 = nn.Linear(in_features=feats // 2, out_features=latent_z, bias=True)
		self.decoder = nn.LSTM(input_size=latent_z, hidden_size=feats // 2, num_layers=1, batch_first=True, bias=True)
		self.dense2 = nn.Linear(in_features=feats // 2, out_features=feats, bias=True)
		assert f_theta in ['seq2seq-att', 'Linear', 'seq2seq-lstm']
		if f_theta == 'seq2seq-att':
			self.predictor = Seq2Seq_with_attention_modified(input_size=latent_z, window=w1, hidden_size=latent_z, output_size=latent_z, plength=w2)
		elif f_theta == 'seq2seq-lstm':
			self.predictor = LSTM_thetafunc(size=latent_z, window=w1, plength=w2)
		elif f_theta == 'Linear':
			self.predictor = Linear_thetafunc(size=latent_z, window=w1, plength=w2)
		else:
			print(f'Error: wrong no type of function {f_theta}, must be one of [\'seq2seq-att\', \'Linear\', \'seq2seq-lstm\']')
		initialParameter(model = self)
  
  
	''' monte carlo method '''
	def _representation(self, mu: torch.Tensor, sigma: torch.Tensor, std: float = 1.0):
		epsilon = torch.randn_like(sigma) * std
		return mu + sigma * epsilon

	def forward(self, X: torch.Tensor, std: float = 1.0):
		
		# print(X.shape)
		# (batch_size, window, input_size // 2)
		temp_z, (_, _) = self.encoder(X)

		# z (batch_size, window, latent_z)
		z = self.dense1(temp_z)
		z1 = torch.clone(z[:, :self.window1, :])
		z2 = torch.clone(z[:, self.window1:, :])
		if self.f_theta == 'seq2seq-att':
			predict_z2, _ = self.predictor(z1)
		elif self.f_theta in ['Linear', 'seq2seq-lstm']:
			predict_z2 = self.predictor(z1)
		else:
			predict_z2 = None
			print('Error: f_theta function must be in [seq2seq-att, Linear, seq2seq-lstm]')	
		# Decode the first windows, w1
		x1, (h_n, c_n) = self.decoder(z1)
		x1 = self.dense2(x1)
		
		# Decode the second windows, w2
		x2, (_, _) = self.decoder(z2, (h_n, c_n))
		x2 = self.dense2(x2)
		if self.rand:
			random_z2 = self._representation(mu=z2, sigma=predict_z2 - z2, std=std)
		else:
			random_z2 = torch.clone(predict_z2)
      
		# decode the second windows, with predictability
		px2, _ = self.decoder(random_z2, (h_n, c_n))
		px2 = self.dense2(px2)
		return x1, x2, px2, z1, z2, predict_z2